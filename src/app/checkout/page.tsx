...
export default function ChequePage() {
  ...
  const [appliedDescuentoId, setAppliedDescuentoId] = useState<number | null>(
    null
  );
          ...
          .map((p: any) => {
            ...
            return {
              ...
              nombre:
                p.nombre ??
                p.title ??
                `Producto ${p.id_producto ?? p.id ?? ""}`,
              precio:
                Number(String(p.precio ?? "0").replace(/[^0-9.-]/g, "")) || 0,
  ...
  const getInitialStock = (id_producto: number) => {
    const p = productosDisponibles.find(
      (x) => Number(x.id_producto) === Number(id_producto)
    );
    ...
    setProductosAgregados((prev) => {
      ...
      if (!found) {
        ...
        if (
          ...
          nuevaQty > (getInitialStock(detalle.id_producto) ?? Infinity)
        ) {
          return prev;
        }
        return prev.map((p) =>
  ...
  const discountObj = useMemo(() => {
    ...
    return (
      descuentos.find(
        (d) => Number(d.id_descuento ?? d.id) === Number(appliedDescuentoId)
      ) ?? null
    );
    ...
    <main className="h-[80vh] flex flex-col mx-4 lg:mx-25">
      <div className="grid grid-cols-1 lg:grid-cols-[3fr_1fr] gap-6 p-4 h-full">
        <div className="bg-light-60 dark:bg-dark-30 rounded-xl p-4 lg:p-6 drop-shadow-xl flex flex-col items-center justify-start gap-4 overflow-y-auto w-full">
          <Button
            className="w-full min-h-14 text-lg font-medium dark:bg-dark-60 dark:hover:bg-neutral-900 dark:text-white bg-light-30 mb-4"
          ...
          {productosAgregados.length === 0 ? (
            ...
            productosAgregados.map((producto, index) => {
              ...
              if (discountObj) {
                const tipoStr = (discountObj.tipo ?? "")
                  .toString()
                  .toUpperCase()
                  .trim();
                const val = Number(discountObj.valor ?? 0);
              ...
              return (
                <div
                  ...
                  className="w-full bg-light-30 dark:bg-dark-60 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 p-4 dark:hover:bg-neutral-900 transition-colors rounded-lg"
                >
                  ...
                  <div className="flex items-center gap-3 w-full lg:w-[25%] justify-start lg:justify-center">
                    ...
                    <button
                      className="p-1 rounded hover:bg-light-10"
                      onClick={() => {
                        const rem = getRemainingStock(producto.id_producto);
                        if (typeof rem === "number" && rem <= 0) {
                          setError("No queda stock disponible.");
                    ...
                    {precioUnitConDesc !== precioUnit ? (
                      <div>
                        ...
                        <div className="dark:text-white">
                          ${(precioUnitConDesc * producto.cantidad).toFixed(2)}
                        </div>
                      </div>
                    ) : (
                      <div className="dark:text-white">
                        ${(precioUnit * producto.cantidad).toFixed(2)}
                      </div>
                    )}
                  </div>
        ...
        <div className="bg-light-60 dark:bg-dark-30 rounded-xl p-4 drop-shadow-xl flex flex-col">
              ...
              {loadingDescuentos ? (
                ...
                <div className="space-y-2 w-full">
                  {descuentos.map((d: any) => {
                    const tipoStr = (d.tipo ?? "PORCENTAJE")
                      .toString()
                      .toUpperCase()
                      .trim();
                    const isMontoFijo = tipoStr === "MONTOFIJO";
                    return (
                      <div
                        ...
                        onClick={() =>
                          applyOrToggleDescuento(Number(d.id_descuento ?? d.id))
                        }
                      >
                        <div>
          ...
          <div
            className={`w-full h-25 mt-4 rounded-md flex items-center justify-center min-h-14 transition-colors ${
              processing
                ? "bg-gray-500 cursor-not-allowed"
                : "bg-light-30 dark:bg-dark-60 hover:bg-green-400 cursor-pointer"
            }`}
            onClick={() => {
              if (!processing) crearVenta();
            }}
          >
            {processing ? (
      ...
      {showPopup && (
        ...
        >
          <div
            className="bg-light-60 dark:bg-dark-30 rounded-xl p-6 w-[90%] max-w-[500px] h-[80%] max-h-[600px] shadow-2xl flex flex-col items-start justify-start relative overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <Search
              id="search-producto"
              placeholder="Buscar nombre de Producto"
              className="w-full bg-light-30 dark:bg-dark-60 text-foreground border-0 focus:ring-0"
              label=""
            />
                ...
                productosDisponibles.map((detalle: any) => {
                  ...
                  return (
                    <div
                      ...
                      className={`w-full bg-light-30 dark:bg-dark-60 hover:dark:bg-neutral-800 hover:bg-light-10 transition-colors rounded-md flex items-center justify-between p-3 mb-2 cursor-pointer ${
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        {typeof detalle.stock === "number" && (
                          <span className="text-xs text-muted-foreground">
                            Stock: {detalle.stock} — Quedan: {remaining}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            if (!disabled) handleAgregarProducto(detalle);
                          }}
