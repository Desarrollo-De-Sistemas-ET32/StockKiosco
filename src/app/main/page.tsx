...
import { ProductoWithId } from "../Service/producto/producto";
...
export default function MainPage() {
        ...
        const ventasNormalizadas: VentaInfo[] = (ventasRaw ?? []).map(
          (v: any) => {
            ...
            const totalFromServer =
              v.total ?? v.total_vendido ?? v.monto ?? null;
            const total =
  ...
  const totalVendidoHoy = ventas
    ...
    .reduce(
      (acc, v) =>
        acc + v.detalles.reduce((a, d) => a + (Number(d.cantidad) || 0), 0),
      0
    );
    ...
    <main className="flex flex-col items-center justify-center gap-10 px-4 sm:px-6 lg:px-10 py-3 lg:mx-50 flex-wrap">
      <div className="w-fit gap-15 grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 justify-items-center inline-grid">
        <InfoCard
          ...
          percentage={
            Number.isFinite(stockBajoPercent) ? +stockBajoPercent.toFixed(1) : 0
          }
        ...
        <InfoCard
          ...
          percentage={
            totalInventario
              ? +((totalProductosVendidos / totalInventario) * 100).toFixed(1)
              : 0
          }
          description="Productos"
        />
